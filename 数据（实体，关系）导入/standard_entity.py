import pymongo
from bson.objectid import ObjectId
import openpyxl
from openpyxl import Workbook
from openpyxl import load_workbook

myclient=pymongo.MongoClient(host='127.0.0.1', port=27017)   #指定主机和端口号创建客户端
mydb = myclient["tech_info"]
mycol = mydb["standard_wrjs_xny"]

wb = Workbook()
ws = wb.active

i = 2
for x in mycol.find():
    standard_num = x["standard_num"]
    ws.cell(row=i, column=7).value = standard_num
    url = x["url"]
    ws.cell(row=i, column=12).value = url
    name = x["name"]
    ws.cell(row=i, column=1).value = name
    ws.cell(row=i, column=6).value = name
    country = x["country"]
    ws.cell(row=i, column=8).value = country
    used_date = x["used_date"]
    ws.cell(row=i, column=9).value = used_date
    public_date = x["public_date"]
    ws.cell(row=i, column=10).value = public_date
    i = i+1

wb.save('standards.xlsx')